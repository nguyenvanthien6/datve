...
CREATE TABLE DinhDangTau --Phim có hỗ trợ màn hình 3D hay IMax không?
(
	id varchar(50) NOT NULL primary key,
	idTau VARCHAR(50) NOT NULL,
	idLoaiTau VARCHAR(50) NOT NULL,

	FOREIGN KEY (idTau) REFERENCES dbo.Phim(id),
	FOREIGN KEY (idLoaiTau) REFERENCES dbo.LoaiManHinh,
)
GO
...
(
	...
	CONSTRAINT PK_PhanLoaiPhim PRIMARY KEY(idTau,idTheLoai)
)
GO
...
(
	...
	FOREIGN KEY (idDinhDang) REFERENCES dbo.DinhDangTau(id),
...
BEGIN
	...
	FROM dbo.Phim P, dbo.DinhDangTau DD
	WHERE @idDinhDang = DD.id AND DD.idTau = P.id
...
BEGIN
	...
	SELECT @count = COUNT(*)
	FROM dbo.LichChieu LC, dbo.DinhDangTau DD, dbo.Phim P
	WHERE LC.idPhong = @idPhong AND LC.idDinhDang = DD.id AND DD.idTau = P.id AND (@ThoiGianChieu >= LC.ThoiGianChieu AND @ThoiGianChieu <= DATEADD(MINUTE, P.ThoiLuong, LC.ThoiGianChieu))

	SELECT @count2 = COUNT(*)
	FROM dbo.LichChieu LC, dbo.DinhDangTau DD, dbo.Phim P
	WHERE @idPhong = LC.idPhong AND @idDinhDang = DD.id AND DD.idTau = P.id AND (LC.ThoiGianChieu >= @ThoiGianChieu AND LC.ThoiGianChieu <= DATEADD(MINUTE, P.ThoiLuong, @ThoiGianChieu))
...
BEGIN
	...
	FROM dbo.Ve AS V, dbo.LichChieu AS LC, dbo.DinhDangTau AS DDP, Phim AS P
	WHERE V.idLichChieu = LC.id AND LC.idDinhDang = DDP.id AND DDP.idTau = P.id AND V.TrangThai = 1 AND P.id = @idMovie AND LC.ThoiGianChieu >= @fromDate AND LC.ThoiGianChieu <= @toDate
...
BEGIN
	...
	WHERE idTau = @idTau AND PL.idTheLoai = TL.id
END
GO
...
BEGIN
	...
	FROM dbo.DinhDangTau DD, Phim P, dbo.LoaiManHinh MH
	WHERE DD.idTau = P.id AND DD.idLoaiTau = MH.id
END
GO

CREATE PROC USP_InsertFormatMovie
@id VARCHAR(50), @idTau VARCHAR(50), @idLoaiTau VARCHAR(50)
AS
BEGIN
	INSERT dbo.DinhDangTau ( id, idTau, idLoaiTau )
	VALUES  ( @id, @idTau, @idLoaiTau )
END
GO
...
BEGIN
	...
	from Phim p ,DinhDangTau d, LichChieu l, PhongChieu pc
	where p.id = d.idTau and d.id = l.idDinhDang and l.idPhong = pc.id
...
BEGIN
	...
	FROM dbo.LichChieu AS LC, dbo.DinhDangTau AS DD, Phim AS P, dbo.LoaiManHinh AS MH
	WHERE LC.idDinhDang = DD.id AND DD.idTau = P.id AND DD.idLoaiTau = MH.id
END
GO
...
BEGIN
	SELECT LC.id AS [Mã lịch chiếu], LC.idPhong AS [Mã phòng], P.TenPhim AS [Tên phim], MH.TenMH AS [Màn hình], LC.ThoiGianChieu AS [Thời gian chiếu], LC.GiaVe AS [Giá vé]
	FROM dbo.LichChieu AS LC, dbo.DinhDangTau AS DD, Phim AS P, dbo.LoaiManHinh AS MH
	WHERE LC.idDinhDang = DD.id AND DD.idTau = P.id AND DD.idLoaiTau = MH.id AND dbo.fuConvertToUnsign1(P.TenPhim) LIKE N'%' + dbo.fuConvertToUnsign1(@tenPhim) + N'%'
END
GO
...
BEGIN
	select l.id, pc.TenPhong, p.TenPhim, l.ThoiGianChieu, d.id as idDinhDang, l.GiaVe, l.TrangThai
	from Phim p ,DinhDangTau d, LichChieu l, PhongChieu pc
	where p.id = d.idTau and d.id = l.idDinhDang and l.idPhong = pc.id and l.TrangThai = 0
	order by l.ThoiGianChieu
END
GO
...
INSERT [dbo].[PhanLoaiPhim] ([idTau], [idTheLoai]) VALUES (N'P01', N'TL01')
INSERT [dbo].[PhanLoaiPhim] ([idTau], [idTheLoai]) VALUES (N'P01', N'TL04')
INSERT [dbo].[PhanLoaiPhim] ([idTau], [idTheLoai]) VALUES (N'P01', N'TL05')
INSERT [dbo].[PhanLoaiPhim] ([idTau], [idTheLoai]) VALUES (N'P02', N'TL01')
INSERT [dbo].[PhanLoaiPhim] ([idTau], [idTheLoai]) VALUES (N'P02', N'TL07')
INSERT [dbo].[PhanLoaiPhim] ([idTau], [idTheLoai]) VALUES (N'P02', N'TL08')
INSERT [dbo].[PhanLoaiPhim] ([idTau], [idTheLoai]) VALUES (N'P03', N'TL03')
INSERT [dbo].[PhanLoaiPhim] ([idTau], [idTheLoai]) VALUES (N'P03', N'TL07')
INSERT [dbo].[PhanLoaiPhim] ([idTau], [idTheLoai]) VALUES (N'P03', N'TL08')
INSERT [dbo].[PhanLoaiPhim] ([idTau], [idTheLoai]) VALUES (N'P04', N'TL02')
INSERT [dbo].[PhanLoaiPhim] ([idTau], [idTheLoai]) VALUES (N'P04', N'TL03')
INSERT [dbo].[PhanLoaiPhim] ([idTau], [idTheLoai]) VALUES (N'P04', N'TL05')

INSERT [dbo].[DinhDangTau] ([id], [idTau], [idLoaiTau]) VALUES (N'DD01', N'P01', N'MH01')
INSERT [dbo].[DinhDangTau] ([id], [idTau], [idLoaiTau]) VALUES (N'DD02', N'P01', N'MH03')
INSERT [dbo].[DinhDangTau] ([id], [idTau], [idLoaiTau]) VALUES (N'DD03', N'P02', N'MH01')
INSERT [dbo].[DinhDangTau] ([id], [idTau], [idLoaiTau]) VALUES (N'DD04', N'P03', N'MH02')
