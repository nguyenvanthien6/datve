...
namespace GUI.DAO
{
    public class TicketDAO
    {
        public static List<Ticket> GetListTicketsByShowTimes(string showTimesID)
        {
            ...
            string query = "select * from Ve where idLichDi = '" + showTimesID + "'";
        ...
        {
            List<Ticket> listTicket = new List<Ticket>();
            string query = "select * from Ve where idLichDi = '" + showTimesID + "' and TrangThai = 1";
            DataTable data = DataProvider.ExecuteQuery(query);
            foreach (DataRow row in data.Rows)
            {
                Ticket ticket = new Ticket(row);
        ...
        {
            string query = "Select count (id) from Ve where idLichDi ='" + showTimesID + "'";
            return (int)DataProvider.ExecuteScalar(query);
        }
        public static int CountTheNumberOfTicketsSoldByShowTime(string showTimesID)
        {
            string query = "Select count (id) from Ve where idLichDi ='" + showTimesID + "' and TrangThai = 1 ";
            return (int)DataProvider.ExecuteScalar(query);
        }
        ...
        {
            string query = "USP_InsertTicketByShowTimes @idLichDi , @maGheNgoi";
        ...
        {
            string query = "USP_DeleteTicketsByShowTimes @idLichDi";
