...
namespace GUI.DAO
{
    public class ScreenTypeDAO
    {
        public static List<ScreenType> GetListScreenType()
        {
            ...
            DataTable data = DataProvider.ExecuteQuery("SELECT * FROM dbo.LoaiVe");
        ...
        {
            return DataProvider.ExecuteQuery("SELECT id AS [Mã loại tàu], TenMH as [Loại phòng] FROM dbo.LoaiVe");
        }

        public static DataTable GetScreenTypeByFormatFilm(string screenTypeID)
        {
            return DataProvider.ExecuteQuery("SELECT *  FROM dbo.LoaiVe where id = '" + screenTypeID + "'");
        }
        ...
        {
            string command = string.Format("UPDATE dbo.LoaiVe SET TenMH = N'{0}' WHERE id = '{1}'", name, id);
        ...
        {
			DataProvider.ExecuteNonQuery("DELETE dbo.PhongTau WHERE idManHinh = '" + id + "'");
			DataProvider.ExecuteNonQuery("DELETE dbo.DinhDangTau WHERE idLoaiTau = '" + id + "'");

			int result = DataProvider.ExecuteNonQuery("DELETE dbo.LoaiVe WHERE id = '" + id + "'");
            return result > 0;
        }
        ...
        public static ScreenType GetScreenTypeByName(string screenName)
        {
            ...
            DataTable data = DataProvider.ExecuteQuery("SELECT * FROM dbo.LoaiVe WHERE TenMH = N'" + screenName + "'");
            foreach (DataRow item in data.Rows)
            {
